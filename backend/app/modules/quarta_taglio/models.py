...
class QuartaTaglioFinalCertificate(Base):
    ...
    unit_key: Mapped[str | None] = mapped_column(String(512), nullable=True, index=True)
    cod_f3: Mapped[str | None] = mapped_column(Text, nullable=True)
    ddt: Mapped[str | None] = mapped_column(Text, nullable=True)
    esolver_id_documento: Mapped[str | None] = mapped_column(String(128), nullable=True, index=True)
    esolver_id_riga_doc: Mapped[str | None] = mapped_column(String(128), nullable=True, index=True)
    esolver_rif_lotto_alfanum: Mapped[str | None] = mapped_column(String(128), nullable=True, index=True)
    ordine_cliente: Mapped[str | None] = mapped_column(Text, nullable=True)
    quantita: Mapped[float | None] = mapped_column(Float, nullable=True)
    cdq_key: Mapped[str | None] = mapped_column(Text, nullable=True, index=True)
    ...
    cert_date: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True, index=True)
